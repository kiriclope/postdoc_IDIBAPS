    ...
    for gv.session in [gv.sessions[-1]] : 
        ...
        for gv.trial in [gv.trials[1]]: 
            ...
            for i, sample in enumerate(gv.samples):
                ...
                X_ED = np.mean(X_avg[:,gv.bins_stim], axis=1) 
                ...
                im = ax.imshow(X_sort, interpolation='lanczos', cmap='jet', vmin=-1.5, vmax=1.5, origin='lower', extent = [-2, gv.duration-2, 0, gv.n_neurons], aspect='auto')
                plt.xlabel('Time (s)')
                plt.ylabel('neuron #')
