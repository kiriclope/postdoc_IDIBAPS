...
def K_fold_clf(clf, X_train, y_train, X_test, y_test, cv): 
    ...
    for train_index, test_index in folds.split(X_train): 
        X_train_train, X_train_test, y_train_train, y_train_test = X_train[train_index], X_train[test_index], y_train[train_index], y_train[test_index] 
...
def mne_cross_temp_clf( X, y, clf=None, cv=10, scoring='accuracy'): 
    if(clf==None): 
        clf = make_pipeline(StandardScaler(), LogisticRegression(C=1,solver='liblinear',penalty='l1')) 
